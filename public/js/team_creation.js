    ...
    // Carica le informazioni degli utenti
     ...
     // Carica e visualizza i giocatori scelti (ora è async)
      ...
      async function confirmSquad() {
        ...
        // Calcola il costo base della squadra
        const baseTeamCost = getTotalCost();
        ...
        console.log("INVIO AL SERVER:", { 
          ...
          selectedMultiplier, 
          multipliedCost,
          totalCost: baseTeamCost // Questo è il valore che dovrebbe essere inviato come totalCost
        });
        ...
        const bodyObj = {
          ...
          totalCost: baseTeamCost // Modifica qui: invia il costo base, non quello moltiplicato
        };
     ...
     document.getElementById('confirmMultiply').addEventListener('click', async () => {
        ...
        const teexBalance = parseFloat(teexBalanceEl.textContent);
         ...
         const bodyObj = {
          ...
          players,
          multiply: selectedMultiplier,
          totalCost: multipliedCost / selectedMultiplier
        };
         try {
          const resp = await fetch("/confirm-squad", {
            method: "POST",
